...
const Navbar = () => {
  const [showMobileMenu, setShwoMobileMenu] = useState(false)
  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
      <div className='container mx-auto flex justify-between item-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
        ...
        <img onClick={()=> setShwoMobileMenu(true)}src={assets.menu_icon} className='md:hidden w-7 cursor-pointer'></img>
      </div>

      {/*----mobile menu----*/}
      <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        <div className='flex justify-end p-6 cursor-pointer'>
          <img onClick={()=> setShwoMobileMenu(false)} src={assets.cross_icon} className='w-6' alt="" />
        </div>
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
          ...
          <a href="#Projects" className='px-4 py-2 rounded-full inline-block'>Project</a>
          <a href="#Projects" className='px-4 py-2 rounded-full inline-block'>Testmonials</a>        
        </ul>
      </div>
    </div>
  )
